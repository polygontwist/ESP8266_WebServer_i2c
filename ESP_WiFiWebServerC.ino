...
const char* ssid = "yours-WLAN-SSID";
const char* password = "yours-WLAN-Password";
...
int noudpcounter=0;
unsigned long stundendifferenz=0;//utc+2
...
void setup() {
 ...
 //get time_diff aus EEPROM
  tempB=readEE(5);
  if(tempB>47)tempB=24;
  if(tempB<1)tempB=24;   //utc+2
  stundendifferenz=tempB-24;  //24=0;  23=-1 25=+1 ...
...
void sendIndexHTM(){
  ...
  const int sPufferMax=1000;//zwischenspeichern für schnellere Überttragung; max ~60000
    ...
    if(s.indexOf("$timeMin") != -1)s.replace("$timeMin",  String(aaktuelleUhrzeit[1]));
     ...
     if(s.indexOf("$eedata") !=-1){//ersten 5 byte im eeprom
      pos3=s.indexOf("$eedata");
      pos4=pos3+7;
       if(pos3>0){
...
void iniEE(){
  ...
  int mm=0;
	...
	//clear
		...
		EEPROM.write(3, 0x66);
		EEPROM.write(4, 0xAB);
		//EEPROM.write(5, 0x00);//reserviert für Zeitdifferenz
    //temp, hum, hh, mm
		//for ( i = 6; i < maxEE; i++)
	  //EEPROM.write(i, 0);
    //defaultwerte
    ...
    for(i=0;i<288;i++){
        adresse=i*4+6;
        EEPROM.write(adresse+0, 0);//temp
        EEPROM.write(adresse+1, 0);//hum
        EEPROM.write(adresse+2, hh);//stunden
        EEPROM.write(adresse+3, mm);//min
    ...
    tempB=EEPROM.read(5);    
    stundendifferenz=tempB-24;  //24=0;  23=-1 25=+1 ...
    if(stundendifferenz>23)stundendifferenz=2;
    if(stundendifferenz<-23)stundendifferenz=2;   
    tempB=stundendifferenz+24;
    EEPROM.write(5,tempB);
